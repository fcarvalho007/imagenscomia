...
final class FCIA_Course {
 ...
 static function receive($req,$kind) {
  ...
  if($kind==='checkout'){
   ...
   $data=array_intersect_key($data,array_flip(array('request_id','edition','name','email','phone','marketing_consent','privacy_acknowledged','terms_acknowledged','session_id','attribution','expected_amount')));
 ...
 static function render() {
  ...
  $html=str_replace('</head>','<meta property="og:image" content="'.esc_url($base.'assets/frederico-carvalho-v20.webp').'"><link rel="stylesheet" href="'.esc_url(add_query_arg('ver','0.5.0',plugins_url('assets/integration.css',__FILE__))).'">'.'</head>',$html);
  ...
  $html=str_replace('</body>','<script>window.FCIA_INTEGRATION='.wp_json_encode($config,JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT).';</script><script defer src="'.esc_url(add_query_arg('ver','0.5.0',plugins_url('assets/integration.js',__FILE__))).'"></script></body>',$html);
  echo $html;exit;
 }
}
