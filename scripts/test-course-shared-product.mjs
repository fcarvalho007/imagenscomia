...
// 26-09-2026: reproduce the current site's two cities sharing a product.
await auth('service_role');
await assert.rejects(
  () => call('sync_course_wp_edition', {...edition,id:'porto-2027-shared-product',modality:'porto',product_id:123}),
  /course_editions_wp_product_unique/,
);
console.log('Confirmed integration blocker: CRM rejects two editions sharing one WooCommerce product. No production changes.');
await db.exec('reset role');
await db.exec(await readFile(new URL('../supabase/migrations/20260926100000_course_shared_product.sql',import.meta.url),'utf8'));
await auth('service_role');
assert.equal((await call('sync_course_wp_edition',{...edition,id:'porto-2027-shared-product',modality:'porto',product_id:123})).accepted,true);checks++;
const shared={...base,edition:'porto-2027-shared-product',order_id:88801,request_id:'bb000000-0000-4000-8000-000000000095',session_id:null,amount_cents:50000,net_cents:40650};
assert.equal((await call('sync_course_woo_order',{...shared,state:'processing',paid:true,paid_at:'2026-09-26T10:00:00Z'})).state,'confirmed');checks++;
assert.equal((await call('sync_course_woo_order',{...shared,state:'processing',paid:true})).duplicate,true);checks++;
await denied(()=>call('sync_course_woo_order',{...shared,revision:2,product_id:98082}));
await denied(()=>call('sync_course_woo_order',{...shared,revision:2,edition:'unknown-2026'}));
assert.equal((await rpc("select count(*)::int n from course_editions where wp_product_id=98080")).n,2);checks++;
assert.equal((await rpc("select amount_cents from course_payments where registration_id=(select id from course_registrations where wp_order_id=88801)")).amount_cents,50000);checks++;
assert.equal((await rpc("select count(*)::int n from course_events where id=$1",[shared.request_id])).n,0);checks++;
await auth('anon');await denied(()=>call('sync_course_woo_order',shared));
await auth('authenticated',true,'aal2');await denied(()=>call('sync_course_woo_order',shared));
console.log(`${checks} total checks passed after shared-product migration; no production orders/messages.`);
await db.close();
