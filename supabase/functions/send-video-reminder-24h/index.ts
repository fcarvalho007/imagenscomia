...
<div style="max-width:600px;margin:0 auto;background:#ffffff;padding:32px 28px;">
  ...
  <p style="color:#333;font-size:16px;line-height:1.6;margin:0 0 8px;">É amanhã. ✅</p>
  <p style="color:#333;font-size:16px;line-height:1.6;margin:0 0 8px;font-weight:700;">Quinta-feira, 5 de Março às 10h00 (Portugal)</p>
  <p style="color:#333;font-size:16px;line-height:1.6;margin:0 0 24px;">Aprende a criar vídeos com IA para marketing — sessão ao vivo, 45–60 min.</p>
  ...
  <p style="color:#555;font-size:14px;text-align:center;margin:0 0 24px;">Vou começar às 10h00 em ponto.<br>Recomendo entrar 5 minutos antes.</p>

  <div style="border-top:1px solid #eee;padding-top:20px;margin-bottom:24px;">
    <p style="color:#333;font-size:16px;font-weight:700;margin:0 0 8px;">⏰ Última oportunidade — preço early bird</p>
    <p style="color:#555;font-size:15px;line-height:1.6;margin:0 0 16px;">A partir do início do webinar, o preço do Premium Pass passa de €15+IVA para €27+IVA.</p>
    <div style="text-align:center;">
      <a href="https://imagenscomia.com/upgrade-video" style="display:inline-block;background:#16a34a;color:#fff;padding:13px 28px;border-radius:8px;font-weight:700;text-decoration:none;font-size:15px;">Garantir agora por €15+IVA →</a>
    </div>
  </div>

  <div style="border-top:1px solid #eee;padding-top:16px;margin-top:32px;">
    <p style="color:#333;font-size:16px;margin:0 0 4px;">Até amanhã,</p>
...
serve(async (req) => {
  ...
  try {
    ...
    const now = Date.now();
    const targetStart = VIDEO_WEBINAR_DATE.getTime() - 25 * 60 * 60 * 1000;
    const targetEnd = VIDEO_WEBINAR_DATE.getTime() - 23 * 60 * 60 * 1000;
    if (now < targetStart || now > targetEnd) {
      return new Response(JSON.stringify({ skipped: true, reason: "Outside 24h window" }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    for (const reg of toSend) {
      try {
        ...
        const emailSubject = tpl?.subject ?? "É amanhã às 10h00 — Vídeo com IA para marketing";
    ...
    console.log(`video_reminder_24h: sent=${sent}, errors=${errors}, skipped=${sentSet.size}`);
