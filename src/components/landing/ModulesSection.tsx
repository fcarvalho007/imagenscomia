      ...
      <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto mb-12">
        {modules.map((mod, i) => (
          <ScrollReveal key={mod.number} delay={i * 0.12}>
            <motion.div
              whileHover={{ y: -6, boxShadow: "0 0 25px hsl(320 80% 60% / 0.15)" }}
              transition={{ type: "spring", stiffness: 300, damping: 20 }}
              className="bg-card rounded-2xl p-7 neon-border hover:border-primary/40 transition-all h-full flex flex-col relative overflow-hidden"
            >
              {mod.badge && (
                <div className="absolute top-4 right-4 text-[10px] font-bold bg-primary/15 text-primary px-2 py-0.5 rounded-full border border-primary/20">
                  {mod.badge}
                </div>
              )}

              <div className="text-3xl mb-3">{mod.emoji}</div>
              <div className="text-xs font-bold text-primary/80 uppercase tracking-wider mb-1">Módulo {mod.number}</div>
              <h3 className="text-lg font-bold mb-3 text-foreground">{mod.title}</h3>

              <p className="text-muted-foreground text-sm leading-relaxed mb-5 flex-1">
                {mod.teaser}
              </p>

              <div className="flex items-center gap-2 text-xs text-primary/70 bg-primary/5 rounded-lg p-3 border border-primary/10">
