...
export default function StatusBlock({ inscrito, onResendLink, onToggleInvoiceSent }: StatusBlockProps) {
  ...
  if (inscrito.payment_status === "awaiting_payment" || inscrito.payment_status === "selected") {
    return (
      <div className="rounded-[10px] p-3" style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.3)" }}>
        <span className="inline-flex items-center gap-1 text-[12px] font-bold" style={{ color: "#d97706" }}>
        ...
        <p className="text-[12px] mt-1.5" style={{ color: "rgba(255,255,255,0.7)" }}>
          {PLAN_LABELS[inscrito.plan_selected || inscrito.plan] || inscrito.plan_selected || "Plano seleccionado"}
        </p>
        {inscrito.payment_link_created_at && (
          <p className="text-[11px] mt-1" style={{ color: linkExpired ? "#ef4444" : "rgba(255,255,255,0.45)" }}>
            {linkExpired
              ? "⚠️ Link expirado"
  ...
  if (inscrito.paid_at) {
    return (
      <div className="rounded-[10px] p-3" style={{ background: "rgba(22,163,74,0.06)", border: "1px solid rgba(22,163,74,0.2)" }}>
        <span className="inline-flex items-center gap-1 text-[12px] font-bold" style={{ color: "#16a34a" }}>
          ✅ Pago
        </span>
        <p className="text-[12px] mt-1.5" style={{ color: "rgba(255,255,255,0.7)" }}>
          {PLAN_LABELS[inscrito.plan] || inscrito.plan} · €{inscrito.valor}
        </p>
        ...
        <div className="mt-2 pt-2" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          {!inscrito.invoice_sent ? (
            <div className="flex items-center justify-between">
              <span className="text-[12px] font-medium" style={{ color: "#d97706" }}>🧾 Fatura por enviar</span>
              {onToggleInvoiceSent && (
                <button
            ...
            <span className="text-[12px]" style={{ color: "rgba(255,255,255,0.4)" }}>🧾 Fatura enviada</span>
          )}
        </div>
      </div>
    );
  ...
  if ((inscrito.step_reached || 0) < 5) {
    ...
    return (
      <div className="rounded-[10px] p-3" style={{ background: "rgba(100,116,139,0.05)", border: "1px solid rgba(100,116,139,0.15)" }}>
        <span className="inline-flex items-center gap-1 text-[12px] font-bold" style={{ color: "#94a3b8" }}>
          Gratuito
        </span>
        <div className="flex items-center gap-1.5 mt-2">
          <span className="text-[12px] font-medium" style={{ color: "rgba(255,255,255,0.7)" }}>Passo {step}/5</span>
  ...
  return (
    <div className="rounded-[10px] p-3" style={{ background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.2)" }}>
      <span className="inline-flex items-center gap-1 text-[12px] font-bold" style={{ color: "#3b82f6" }}>
        ✓ Flow completo
      </span>
