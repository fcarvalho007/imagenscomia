...
export default function RecursosVideoLogin({ onAuthed }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (ADMIN_EMAILS.includes(normalizedEmail)) {
      sessionStorage.setItem("recursos_video_token", "admin");
      sessionStorage.setItem("recursos_video_email", normalizedEmail);
      sessionStorage.setItem("recursos_video_plan", "bundle");
      sessionStorage.setItem("recursos_video_name", "Equipa");
    ...
    try {
      ...
      sessionStorage.setItem("recursos_video_token", data.edit_token ?? "");
      sessionStorage.setItem("recursos_video_email", email.toLowerCase().trim());
      sessionStorage.setItem("recursos_video_plan", data.plan_selected ?? "");
      sessionStorage.setItem("recursos_video_name", data.first_name ?? "");
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-[hsl(var(--ink-500))] mt-2 text-sm">
            Gravação · Guia de apoio · Biblioteca de prompts
          </p>
        </div>
        ...
        <p className="text-center text-xs text-[hsl(var(--ink-400))] mt-6">
          Área privada para participantes pagos.
        </p>
      </div>
    </div>
  );
