...
export default function CourseCheckout() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!config?.enabled || !quote || busy) return;
  ...
  return <div className="course-checkout-demo course-checkout-live"><div className="checkout-demo-shell">
    ...
    ].map(([name, label, auto, min]) => <label key={name}>{label}<input name={String(name)} autoComplete={String(auto)} required minLength={Number(min)} maxLength={name === "tax_id" ? 9 : 180} pattern={name === "tax_id" ? "[0-9]{9}" : name === "postal_code" ? "[0-9]{4}-[0-9]{3}" : undefined} /></label>)}</div><p>Para faturação fora de Portugal, contacte {support}.</p><button className="checkout-live-submit" disabled={busy}>{busy ? "A guardar…" : "Guardar dados de faturação"}</button></form>}</section> : !config?.enabled ? <section className="checkout-live-feedback"><h2>Inscrições online disponíveis em breve.</h2><p>Para saber mais sobre esta edição, contacte {support}.</p></section> : <form className="checkout-demo-layout" onSubmit={submit}>
      <div className="checkout-demo-content"><section className="checkout-demo-course"><h2>A edição que escolheu</h2><label className="checkout-demo-select">Edição<select value={edition} disabled={busy || submitted.current} onChange={e => { setEdition(e.target.value as Edition); requestId.current = crypto.randomUUID(); }}>{EDITIONS.map(id => <option key={id} value={id}>{editions[id].label}</option>)}</select></label><p className="checkout-demo-schedule">{selected.dates}<span>{selected.place}<br />{selected.schedule}</span></p></section>
      <section className="checkout-live-form"><h2>Os seus dados</h2><div className="checkout-live-fields"><label>Nome completo<input name="name" autoComplete="name" required minLength={2} maxLength={120} readOnly={busy || submitted.current} /></label><label>Email<input name="email" type="email" autoComplete="email" required maxLength={254} readOnly={busy || submitted.current} /></label><label>Telemóvel <span>(opcional)</span><input name="phone" type="tel" autoComplete="tel" maxLength={30} readOnly={busy || submitted.current} /></label></div><label className="checkout-live-honey" aria-hidden="true">Website<input name="website" tabIndex={-1} autoComplete="off" /></label>
      <label className="checkout-live-check"><input name="privacy" type="checkbox" required /><span>Li a <a href={config.privacyUrl} target="_blank" rel="noreferrer">política de privacidade</a>.</span></label><label className="checkout-live-check"><input name="terms" type="checkbox" required /><span>Aceito as <a href={config.termsUrl} target="_blank" rel="noreferrer">condições de inscrição, alteração e cancelamento</a>.</span></label><label className="checkout-live-check"><input name="sms" type="checkbox" /><span>Quero receber os lembretes do curso por SMS. Opcional.</span></label><label className="checkout-live-check"><input name="marketing" type="checkbox" /><span>Quero receber novidades sobre futuras formações. Opcional.</span></label></section></div>
      <aside className="checkout-live-summary"><h2>O seu curso</h2><strong>{selected.label}</strong><p>{selected.dates}</p><ul>{["14 horas de formação em direto", "Sessão individual antes e depois", "Templates, checklists e recursos", "Videoaulas de resumo", ...(edition === "online-2026" ? ["Gravações disponíveis durante 1 ano"] : [])].map(item => <li key={item}><Check size={17} aria-hidden="true" />{item}</li>)}</ul><dl><div><dt>Curso</dt><dd>{quote ? money(quote.net_cents) : "—"}</dd></div><div><dt>IVA (23%)</dt><dd>{quote ? money(quote.amount_cents - quote.net_cents) : "—"}</dd></div><div className="checkout-live-total"><dt>Total a pagar</dt><dd>{quote ? money(quote.amount_cents) : "—"}</dd></div></dl><button className="checkout-live-submit" disabled={!quote || busy}>{busy ? "A preparar o pagamento…" : "Continuar para pagamento"}<ArrowRight size={18} aria-hidden="true" /></button><p className="checkout-live-secure"><LockKeyhole size={15} aria-hidden="true" />Cartão, MB WAY ou Multibanco através da Eupago.</p></aside>
