...
export default function CourseCheckout() {
  ...
  return <div className="course-checkout-demo course-checkout-live"><div className="checkout-demo-shell">
    ...
    {loading ? <p role="status">A consultar a disponibilidade…</p> : returning && paymentState !== "paid" ? <section className="checkout-live-feedback"><h2>{paymentState === "checking" ? "A verificar o pagamento…" : closedPayments[paymentState]?.[0] || "Confirmação de pagamento pendente"}</h2><p>{paymentState === "missing" ? "Abra a confirmação no navegador onde iniciou a inscrição, ou contacte o suporte." : closedPayments[paymentState]?.[1] || "Se escolheu Multibanco, a confirmação só acontece depois de pagar a referência. Não repita o pagamento enquanto aguarda."}</p><p>{support}</p></section> : paymentState === "paid" ? <section className="checkout-live-form"><h2>Dados para a fatura</h2>{billingSaved ? <p role="status">Dados guardados. A fatura será enviada para o email da inscrição após emissão.</p> : <form onSubmit={saveBilling}><div className="checkout-live-fields">{[
      ...
      <div className="checkout-step-header"><CheckoutProgress labels={["Edição", "Os seus dados", "Revisão"]} current={step===0?0:step===5?2:1}/><p role="status">Passo {step+1} de {stepTitles.length}</p><progress value={step+1} max={stepTitles.length} aria-label="Progresso da inscrição"/><h2 ref={stepHeading} tabIndex={-1}>{stepTitles[step]}</h2></div>
      ...
      <aside className="checkout-live-summary"><h2>O seu curso</h2><strong>{selected.label}</strong><p>{selected.dates}</p><details className="checkout-inclusions"><summary>O que está incluído</summary><ul>{["14 horas de formação em direto", "Sessão individual antes e depois", "Templates, checklists e recursos", "Videoaulas de resumo", ...(edition === "online-2026" ? ["Gravações disponíveis durante 1 ano"] : [])].map(item => <li key={item}><Check size={17} aria-hidden="true" />{item}</li>)}</ul></details><dl><div><dt>Curso</dt><dd>{quote ? money(quote.net_cents) : "—"}</dd></div><div><dt>IVA (23%)</dt><dd>{quote ? money(quote.amount_cents - quote.net_cents) : "—"}</dd></div><div className="checkout-live-total"><dt>Total a pagar</dt><dd>{quote ? money(quote.amount_cents) : "—"}</dd></div></dl><button className="checkout-live-submit" hidden={step!==5} disabled={!quote || busy}>{busy ? "A preparar o pagamento…" : "Continuar para pagamento"}<ArrowRight size={18} aria-hidden="true" /></button><p className="checkout-live-secure"><LockKeyhole size={15} aria-hidden="true" />Cartão, MB WAY ou Multibanco através da Eupago.</p></aside>
